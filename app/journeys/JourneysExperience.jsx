...
const arcProfiles = [
  ...
  { id: "disconnected", feeling: "Disconnected", name: "The Return", territory: "Spiritual homecoming", status: "Launched", copy: "For the person who is present everywhere and connected nowhere. A private journey designed around reconnection with the world, the people you love and yourself.", who: "You want to feel present again, and would rather shape the route in conversation than choose from a fixed itinerary." },
...
function JourneyCard({ journey }) {
  return <Link className="jnew-card" href={`/journeys/${journey.slug}`}>
    ...
    <div className="jnew-card-body"><div className="jnew-card-meta"><span>{journey.nights} nights</span><span>{journey.destination}</span></div><h3>{journey.slug === "the-reset" ? "The Reset" : journey.nights === 6 ? "Pure Decompression" : journey.nights === 9 ? "Beach, Then Wilderness" : "The Complete Restoration"}</h3><p>{journey.description}</p><div className="jnew-card-footer"><span>From <strong>{journey.price}</strong> / person</span><b>Explore →</b></div></div>
  </Link>;
}
...
export default function JourneysExperience({ journeys }) {
  ...
  return <main className="journeys-new">
    ...
    {view === "journeys" && <div className="jnew-main">
      ...
      {showDisconnected && <section className="jnew-arc jnew-disconnected" id="disconnected"><div className="jnew-arc-heading"><div><span>02 · Disconnected · Launched</span><h2>The <em>Return</em></h2><small>Spiritual homecoming · private design</small></div><div><p>{arcProfiles[1].copy}</p><Link href="/request?interest=Disconnected%2C%20The%20Return&arc=disconnected">Begin a private conversation →</Link></div></div><div className="jnew-arc-note"><p>No pre-set itinerary. We shape the pace, place and experiences with you in a private conversation.</p></div></section>}
