...
export async function GET(request) {
  ...
  try {
    ...
    const listCondition = view === "trash" ? "deleted_at IS NOT NULL" : "deleted_at IS NULL AND archived_at IS NULL AND is_spam = 0";
    const [counts, enquiries] = await db.batch([
      db.prepare(`SELECT COUNT(*) AS total,
        ...
        FROM journey_enquiries WHERE deleted_at IS NULL AND archived_at IS NULL AND is_spam = 0`),
      db.prepare(`SELECT id, reference, created_at, name, email, feeling, travel_month, travel_year,
        ...
        FROM journey_enquiries WHERE ${listCondition}
        ORDER BY created_at DESC LIMIT 200`),
    ]);
    const deleted = await db.prepare("SELECT COUNT(*) AS total FROM journey_enquiries WHERE deleted_at IS NOT NULL").first();
    const summary = { ...(counts.results?.[0] || {}), deleted_count: Number(deleted?.total || 0) };
    return Response.json({ actor: actor.email, counts: summary, enquiries: enquiries.results || [], view }, { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
