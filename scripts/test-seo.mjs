...
check(homepageExperience.includes('setInterval') && homepageExperience.includes('6000') && homepageExperience.includes('Pause automatic sliding') && homepageExperience.includes('IntersectionObserver') && homepageExperience.includes('document.hidden') && homepageExperience.includes('onFocusCapture') && homepageExperience.includes('requestAnimationFrame(glide)') && homepageExperience.includes('cinematic-slide') && homepageExperience.includes('inert={!active'), "Full-frame journey slideshow glides automatically with pause, visibility and keyboard focus safeguards");
...
check(!journeyIndex.includes('id="stillness"') && journeyIndex.includes('router.push("/journeys/stillness")') && journeys.filter((journey) => journey.arc === "stillness").length === 7, "Journey index omits its Stillness section and routes both Stillness filter choices to the dedicated seven-tour page");
...
const feelingLedPage = read("app/transformational-travel/page.jsx");
check(feelingLedPage.includes('<div className="new-home fl-feeling-quiz"><FeelingQuiz /></div>'), "Transformational feeling quiz has its shared styling scope");
check(feelingLedPage.includes('title: "Transformational Travel Designed Around Feeling | Ryravel"') && feelingLedPage.includes('description: "Transformational travel that starts with a feeling, not a destination.') && feelingLedPage.includes('absoluteTitle: true'), "Emotion-led landing page has its requested canonical title and description");
check(feelingLedPage.includes("Transformational travel, designed around how you want to feel") && feelingLedPage.includes("Most transformational travel sells you an itinerary.") && feelingLedPage.includes("<FeelingQuiz />") && feelingLedPage.includes('/journeys/stillness'), "Emotion-led page explains the method, proves it with Stillness and reuses the feeling quiz");
check(read("app/components/HomepageExperience.jsx").includes('["Emotion-led journeys", "/transformational-travel"]') && read("app/travel-styles/page.jsx").includes('href="/transformational-travel"') && read("app/components/SiteChrome.jsx").includes('href="/transformational-travel"') && read("app/sitemap.js").includes('["/transformational-travel"'), "Emotion-led page is linked from homepage, travel styles and footer, and included in the sitemap");
...
check(requestPage.includes("What happens now") && requestPage.includes("Maryangel reviews the feeling") && requestPage.includes("initial investment direction"), "Request page explains the personal post-enquiry process");
...
check(requestPage.includes("A note from Maryangel") && requestPage.includes("Maryangel Ajuzieogu") && requestPage.includes("founderNoteRef.current?.showModal()"), "Request flow shows the founder and makes her complete note available without lengthening the fixed form");
...
check(read("app/lib/delivery-providers.js").includes("RESEND_API_KEY") && read("app/api/enquiries/route.js").includes('deliveryStatement(db, id, "confirmation"'), "Branded guest confirmation email is integrated without blocking submission");
check(read("app/lib/delivery-providers.js").includes("This is an automatic receipt") && read("app/lib/delivery-providers.js").includes("Maryangel Ajuzieogu"), "Automated receipt clearly distinguishes itself from Maryangel's personal response");
...
check(read("app/lib/delivery-providers.js").includes("curator@updates.ryravel.com"), "Confirmation email sends from the Resend-verified Ryravel subdomain");
...
check(stillness?.intro.startsWith("Seven wilderness journeys") && journeys.filter((journey) => journey.arc === "stillness").length === 7, "Stillness Collection introduction matches its seven journeys");
const calvingSeason = journeys.find((journey) => journey.slug === "calving-season");
check(Boolean(calvingSeason) && calvingSeason.arc === "disconnected" && calvingSeason.nights === 6 && calvingSeason.days === 7, "Calving Season is the six-night launched Disconnected journey");
...
check(calvingSeason.faqs.length >= 4 && calvingPage.includes("JourneyAvailability") && calvingPage.includes("JourneyFaqs") && calvingPage.includes('/journeys/disconnected') && !calvingPage.includes("JourneyProofSection") && !calvingPage.includes("StillnessAlternatives") && calvingPage.includes("Request a private call"), "Calving Season has Disconnected navigation and planning without unrelated case-study proof");
check(journeys.filter((journey) => journey.arc === "disconnected").length === 3 && read("app/journeys/JourneysExperience.jsx").includes('journey.arc === "disconnected"') && !read("app/journeys/[slug]/JourneyPlanning.jsx").includes('"calving-season"') && !read("app/journeys/[slug]/JourneyPlanning.jsx").includes('"calving-isolation"') && !read("app/journeys/[slug]/JourneyPlanning.jsx").includes('"full-calving-arc"'), "All three calving journeys appear only in the Disconnected catalogue and not Stillness alternatives");
check(read("app/journeys/[slug]/page.jsx").includes('cards.length === 1 ? "journey" : "journeys"') && read("app/journeys/[slug]/page.jsx").includes('arc.id !== "disconnected" && <JourneyProof'), "Disconnected arc uses singular journey language and omits unrelated case-study proof");
const calvingIsolation = journeys.find((journey) => journey.slug === "calving-isolation");
check(Boolean(calvingIsolation) && calvingIsolation.arc === "disconnected" && calvingIsolation.nights === 7 && calvingIsolation.days === 8, "Calving Maximum Isolation is a first-class seven-night Disconnected journey");
...
check(calvingIsolation.faqs.length >= 4 && calvingIsolationPage.includes("JourneyAvailability") && calvingIsolationPage.includes("JourneyFaqs") && calvingIsolationPage.includes('/journeys/disconnected') && !calvingIsolationPage.includes("JourneyProofSection") && !calvingIsolationPage.includes("StillnessAlternatives") && calvingIsolationPage.includes("Request a private call"), "Calving Maximum Isolation has Disconnected navigation and planning without unrelated case-study proof");
const fullCalvingArc = journeys.find((journey) => journey.slug === "full-calving-arc");
check(Boolean(fullCalvingArc) && fullCalvingArc.arc === "disconnected" && fullCalvingArc.nights === 9 && fullCalvingArc.days === 10, "The Full Calving Arc is a first-class nine-night Disconnected journey");
...
check(fullCalvingArc.faqs.length >= 4 && fullCalvingArcPage.includes("JourneyAvailability") && fullCalvingArcPage.includes("JourneyFaqs") && fullCalvingArcPage.includes('/journeys/disconnected') && !fullCalvingArcPage.includes("JourneyProofSection") && !fullCalvingArcPage.includes("StillnessAlternatives") && fullCalvingArcPage.includes("Request a private call"), "The Full Calving Arc has Disconnected navigation and planning without unrelated case-study proof");
...
const trustLayer = read("app/components/JourneyTrustLayer.jsx");
const publishedJourneyTemplates = ["page", "ExhaustedRestoration", "ExhaustedRestorationSix", "ExhaustedRestorationNine", "ResetJourney", "CalvingSeasonJourney", "CalvingIsolationJourney", "FullCalvingArcJourney", "KimbilioJourney", "KimyaJourney", "KuponaJourney", "RunyararoJourney", "UtalalaJourney"];
check(publishedJourneyTemplates.every((name) => { const source = read(`app/journeys/[slug]/${name}.jsx`); return source.includes("<JourneyPricingNote />") && source.includes("<JourneyTrustLayer />"); }), "Every published journey template places pricing disclosure and booking trust beside the investment");
check(trustLayer.includes("50% deposit") && trustLayer.includes("30 days before departure") && trustLayer.includes("within 45 days") && trustLayer.includes("24/7 support") && trustLayer.includes("/cancellations") && trustLayer.includes("visa fees") && trustLayer.includes("supplier"), "Journey trust layer covers payment, changes, support, add-ons and suppliers");
check(read("app/terms/page.jsx").includes("service agreement") && read("app/terms/page.jsx").includes("cleared payment") && read("app/terms/page.jsx").includes("30 days before departure") && read("app/terms/page.jsx").includes("within 45 days") && read("app/cancellations/page.jsx").includes("until 15 days"), "Booking and amendment terms match the new published timeline");
