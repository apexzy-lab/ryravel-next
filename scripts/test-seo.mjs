...
check(read("app/journeys/page.jsx").includes("<JourneysExperience journeys={journeys} />") && read("app/sitemap.js").includes("journeys.some"), "Journey catalogue includes all active tours and collection routes remain discoverable");
...
check(journeyIndex.includes('journey.arc === "stillness"') && journeys.filter((journey) => journey.arc === "stillness").length === 7 && ["kimbilio", "kimya", "kupona", "runyararo", "utalala", "st6", "st9"].every((slug) => journeys.some((journey) => journey.slug === slug)), "Journey catalogue features all seven current Stillness tours without retired tour records");
