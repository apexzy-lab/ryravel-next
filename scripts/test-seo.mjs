...
check(stillness?.intro.startsWith("Nine wilderness journeys") && journeys.filter((journey) => journey.arc === "stillness").length === 9, "Stillness Collection introduction matches its nine journeys");
...
check(journeys.filter((journey) => journey.arc === "disconnected").length === 1 && read("app/journeys/JourneysExperience.jsx").includes('journey.arc === "disconnected"') && !read("app/journeys/[slug]/JourneyPlanning.jsx").includes('"calving-season"'), "Calving Season appears only in the Disconnected catalogue and not Stillness alternatives");
